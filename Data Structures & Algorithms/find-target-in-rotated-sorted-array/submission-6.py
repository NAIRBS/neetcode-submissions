class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        while left <= right:
            ...
            # This also works, but it is inclusion based instead of exclusion based
            ...
            #         right = middle - 1  # Search left, eliminate right
            #     else:
            ...
            #         left = middle + 1  # Search right, eliminate left
            #     else:
            #         right = middle - 1  # Search left as target is in the jumbled up left side

            if nums[left] <= nums[middle]: # If the LEFT side is sorted
                if target > nums[middle] or target < nums[left]: # Check if target is OUTSIDE sorted left half
                    left = middle + 1 # Throw away left half, search right
                else:
                    right = middle - 1 # Target fits in the left half, search left
            else: # If the RIGHT side is sorted
                if target < nums[middle] or target > nums[right]: # Check if target is OUTSIDE sorted right half
                    right = middle - 1 # Throw away right half, search left
                else:
                    left = middle + 1 # Target fits in the right half, search right
        return -1